...
int main() {
    ...
    // The assignment suggests testing with varying sizes (from 1K to 1M). For demonstration, we'll use 1000.
    int n = 1000;
    int iterations = 10;
    
    // Generate input datasets.
    int* sorted_array = generate_sorted_array(n);
    int* reverse_sorted_array = generate_reverse_sorted_array(n); // Descending order.
    int* random_array = generate_random_array(n);
    int* partially_sorted_array = generate_partially_sorted_array(n);
    
    // Run experiments using Tim Sort.
